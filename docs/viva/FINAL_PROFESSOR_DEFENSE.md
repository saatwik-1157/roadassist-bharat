...
| A user can only reach their own data | 12 cross-tenant attacks refused | `apps/api/src/server.ts:719` `bookingAudience()` | — (run the suite) | **M2 multitenancy** |
| Diagnosis is a labelled rules engine, not a model | Response carries `rules-1.0.0` | `apps/api/src/server.ts:602` → `domain/ai-rules.ts` | Diagnosis, 2:15 | M1 service boundary |
...
| One job is assigned exactly once | 10 simultaneous accepts → 1 winner | `apps/api/src/server.ts:944` — `SELECT … FOR UPDATE` inside `db.transaction` | Mechanic accept, 4:00 | M4 workload distribution |
...
| The client never decides money arrived | Amount is the invoice total; signature recomputed | `apps/api/src/server.ts:1149` (pay), `:1318` (webhook) | Payment, 5:30 | M2 service tech |
| A booking cannot be PAID without a settled payment | 409 `payment_required` | `apps/api/src/server.ts:1025` | Payment, 5:30 | M2 service tech |
...
| Readiness is real | DB down → `/health` 503, `/v1/ping` 200, auto-recovery | `apps/api/src/server.ts:357` | Chaos, optional | **M4 dynamic scalability** |
