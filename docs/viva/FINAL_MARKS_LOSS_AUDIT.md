...
| 4 | Test-count claim said 600 across seven suites (old figure) | One suite that was never part of the run — 22 payment checks — made a headline number unsupportable | Corrected to the six executed suites — 757 as re-measured on 2026-09-12 — in six documents and the deck. The 22 are listed separately as not executed | **FIXED** |
| 5 | Route count stated as "65 endpoints under `/v1`" (old wording)  <!-- claims-check:ignore: quotes the corrected-away figure on purpose --> | Today 64 are under `/v1` and 67 is the total including `/health`, `/tiles` and `/basemap`. Small, but a professor who counts will find it | Reworded in four documents | **FIXED** |
...
| 10 | **No independent penetration test** | "Have you had a pentest?" — no | Say so plainly: 101 self-written attacks (74 security + 27 gateway-security) is not the same thing. Do not dress it up | **ACCEPTED — answer prepared** |
| 11 | **Eight cloud concepts are DESIGN** | The demo is deployed — one Render instance + Neon, Singapore — but an examiner wanting autoscaling, replication or load balancing may mark down regardless of quality | Lead with the two Module 4 architectures that *are* built and the readiness gate; then name the three in-process blockers precisely. Precision reads as competence; vagueness reads as bluffing | **ACCEPTED — answer prepared** |
| 12 | **The AI is if-statements** | "Your AI is a lookup table" | Agree immediately, then pivot to the two genuinely interesting parts: the safety asymmetry (a model may make a verdict stricter, never laxer) and the device/cloud divergence guard that fails CI. Mention the real trained YOLO11 detectors: best run YOLO11s (`yolo11s-multi-rich`) mAP50 0.472; the YOLO11n India model whose detections RAKSHA shows scored mAP50 0.443 | **ACCEPTED — answer prepared** |
...
| 16 | Payments never touched a real gateway | "Did you actually take a payment?" | No — `mock` on the deployment and a local stub in tests, and the deck says so. The stub speaks Razorpay's real wire format and signatures are recomputed server-side | **ACCEPTED** |
...
1. **"Is anything deployed?"** — Yes, as a demo: one Render web service and a
   Neon Postgres database, both in Singapore, at
   https://app.roadassistbharat.online. It is a single instance on a free plan —
   no cluster, no autoscaling, no replication — and it is in Singapore because
   the free tiers have no India region. I would rather say that than point at a
   diagram.
