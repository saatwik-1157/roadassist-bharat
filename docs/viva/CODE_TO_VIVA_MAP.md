...
| Authorization | `apps/api/src/server.ts:719` | `bookingAudience()` | Customer, assigned mechanic, or admin — nobody else | "Ownership is checked on the resource, not inferred from the route." |
...
| Incident creation | `apps/api/src/server.ts:639` | `POST /v1/bookings` | Validates, persists, audits, then dispatches | "Validated by zod, persisted, audited, then dispatch starts — in that order." |
| AI | `apps/api/src/server.ts:602` → `domain/ai-rules.ts` | `POST /v1/diagnose` | Deterministic rule table | "It returns `rules-1.0.0`. I won't call it AI." |
...
| Concurrency | `apps/api/src/server.ts:944` | `SELECT … FOR UPDATE` inside `db.transaction` | Serialises acceptance on the booking row | "Ten simultaneous accepts: one winner, nine clean refusals." |
...
| Payment | `apps/api/src/server.ts:1149` | `POST /v1/bookings/:id/pay` | Amount is the invoice total | "The client never sends the amount." |
| Webhook | `apps/api/src/server.ts:1318` | `POST /v1/webhooks/razorpay` | Recomputes the signature server-side | "A forged signature settles nothing — 402." |
| Payment guard | `apps/api/src/server.ts:1025` | transition check | Refuses PAID without settlement | "409 `payment_required`." |
...
| Monitoring | `apps/api/src/server.ts:357` + `observability.ts` | `/health`, `/v1/ops/overview` | Readiness, live counts, chain verification | "Health separates application from database — that is the readiness gate." |
