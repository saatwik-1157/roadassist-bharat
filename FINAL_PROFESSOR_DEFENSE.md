> Every line numbers a real file, checked on 2026-09-06. Open the file, do not
...
| A user can only reach their own data | 12 cross-tenant attacks refused | `apps/api/src/server.ts:832` `bookingAudience()` | — (run the suite) | **M2 multitenancy** |
| Diagnosis is a labelled rules engine, not a model | Response carries `rules-1.0.0` | `apps/api/src/server.ts:715` → `domain/ai-rules.ts` | Diagnosis, 2:15 | M1 service boundary |
...
| One job is assigned exactly once | 10 simultaneous accepts → 1 winner | `apps/api/src/server.ts:1057` — `SELECT … FOR UPDATE` inside `db.transaction` | Mechanic accept, 4:00 | M4 workload distribution |
...
| SOS creates an incident and reports each rung as fact | Escalation card shows real counts | `apps/api/src/server.ts:2350` | Online SOS, 6:15 | M1 SaaS |
...
| Sync is idempotent | Re-sync creates nothing | `apps/api/src/server.ts:2654` + unique `client_incident_id` | Sync, 8:00 | M4 migration |
| The client never decides money arrived | Amount is the invoice total; signature recomputed | `apps/api/src/server.ts:1262` (pay), `:1431` (webhook) | Payment, 5:30 | M2 service tech |
| A booking cannot be PAID without a settled payment | 409 `payment_required` | `apps/api/src/server.ts:1141` | Payment, 5:30 | M2 service tech |
| History cannot be edited | Postgres RULES make UPDATE/DELETE no-ops | `apps/api/src/audit.ts:83` `audit()`, `:122` `verifyAuditChain()` | `/v1/ops/overview` | M3 monitoring |
| Readiness is real | DB down → `/health` 503, `/v1/ping` 200, auto-recovery | `apps/api/src/server.ts:354` | Chaos, optional | **M4 dynamic scalability** |
| Production refuses unsafe config | Boots refused on 8 settings | `apps/api/src/env.ts:286` `assertProductionSafe()` | Terminal | M1 boundaries |
...
| 578 assertions, 0 failures | Six suites, executed twice | `FINAL_REPOSITORY_STATUS.md` | Terminal | — |
